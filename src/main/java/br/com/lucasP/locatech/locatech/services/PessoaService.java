...
import br.com.lucasP.locatech.locatech.repositories.PessoaRepository;
...
public class PessoaService {
    ...
    public void savePessoa(Pessoa pessoa){
        ...
        Assert.state(save == 1, "Ocorreu um erro ao salvar a pessoa " + pessoa.getNome());
    }

    public void updatePessoa(Pessoa pessoa, Long id){
        var update = this.pessoaRepository.update(pessoa, id);
        if (update == 0) {
            throw new RuntimeException("Ocorreu um erro ao atualizar a pessoa " + pessoa.getNome());
        }
    }
    ...
    public void deletePessoa(Long id){
        ...
        if (delete == 0) {
            throw new RuntimeException("Ocorreu um erro ao deletar a pessoa com id " + id);
        }
    }
}
